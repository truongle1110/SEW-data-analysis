...
d = df2['Maintenance duration']
ID_activity = df2['ID activity']
ID_component = df2['ID component']

map_activity_to_IDcomponent = list(zip(ID_activity, ID_component))      # list of tuple (ID_activity, ID_component)   
map_activity_to_replacement_time = list(zip(ID_activity, t))            # list of tuple (ID_activity, t)
...
# mapping group of activity to group of duration using list of tuple map_activity_to_duration defined above
def mapping_activity_to_duration(map_activity_to_duration, G_activity):
    # Create a dictionary to map each activity to its replacement time t
    dict_map = {activity: duration for activity, duration in map_activity_to_duration}
    # Initialize the result list
    group_to_duration = []
    # Process each group and its activities
    for group, activities in G_activity:
        # Find the time list for each activity in the current group
        duration_list = [dict_map[activity] for activity in activities if activity in dict_map]
        # Append the result as a tuple (group, time list)
        group_to_duration.append((group, duration_list))
    total_duration = [sum(durations) for _, durations in group_to_duration]
...
genome = [123, 117, 116, 116, 20, 20, 11, 11, 114, 113, 110, 111, 109, 108, 106, 105, 103, 45, 45, 45, 101, 101, 98, 96, 95, 90, 92, 86, 60, 60, 60, 14, 14, 43, 43, 85, 83, 81, 79, 28, 28, 78, 76, 4, 4, 77, 77, 73, 69, 67, 64, 74, 74, 61, 119, 119, 57, 56, 59, 59, 59, 12, 12, 91, 91, 54, 52, 51, 50, 35, 35, 35, 47, 47, 49, 48, 46, 44, 42, 41, 41, 30, 25, 118, 118, 22, 37, 37, 37, 19, 18, 1, 1, 1, 99, 99, 99, 17, 39, 39, 39, 39, 39, 89, 89, 16, 15, 10, 82, 82, 65, 65, 9, 27, 27, 84, 84, 84, 84, 7, 6, 2, 5]
        # Best fitness = 3888.5624167402934
N, G_activity = decode(genome)
